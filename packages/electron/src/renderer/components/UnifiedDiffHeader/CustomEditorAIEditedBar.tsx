...
const aiEditedBarRootClasses =
  'unified-diff-header agent-elements-custom-editor-ai-edited-bar agent-elements-tool-card sticky top-0 left-0 right-0 z-[100] border-b border-nim bg-nim-secondary shadow-[0_12px_32px_color-mix(in_srgb,var(--nim-text)_8%,transparent)] @container/diff-header';

const aiEditedBarButtonClasses =
  'unified-diff-header-button agent-elements-unified-diff-header-button flex items-center gap-2 rounded-[8px] border border-[var(--nim-primary)] bg-[var(--nim-primary)] px-3 py-1.5 text-[13px] font-medium leading-5 text-[var(--nim-bg)] cursor-pointer whitespace-nowrap transition-[background-color,color,border-color,opacity] duration-150 hover:enabled:opacity-90 focus-visible:outline-2 focus-visible:outline-[var(--nim-primary)] focus-visible:outline-offset-2';

const aiEditedSessionButtonClasses =
  'unified-diff-header-goto agent-elements-custom-editor-ai-session flex items-center gap-1.5 rounded-[8px] border border-nim bg-nim px-2 py-1 text-[13px] text-nim-muted cursor-pointer transition-[background-color,color,border-color] duration-150 hover:bg-nim-hover hover:text-nim focus-visible:outline-2 focus-visible:outline-[var(--nim-primary)] focus-visible:outline-offset-1';
      ...
      <div className="unified-diff-header-content flex min-h-[48px] items-center justify-between gap-3 px-4 py-2 @[max-450px]/diff-header:flex-wrap @[max-450px]/diff-header:gap-2 @[max-450px]/diff-header:px-3 @[max-350px]/diff-header:px-2">
        ...
        <div className="unified-diff-header-info flex min-w-0 shrink items-center gap-3 overflow-hidden @[max-450px]/diff-header:flex-[1_1_100%]">
          {sessionInfo?.sessionTitle ? (
            <div className="unified-diff-header-session agent-elements-custom-editor-ai-edited-session flex min-w-0 items-center gap-2 overflow-hidden text-[13px] text-nim">
              <MaterialSymbol icon="smart_toy" size={18} className="unified-diff-header-session-icon shrink-0 text-[var(--nim-primary)]" />
              <div className="unified-diff-header-session-details flex min-w-0 items-center gap-1.5">
                <span className="unified-diff-header-label agent-elements-custom-editor-ai-label flex items-center gap-2 text-[13px] font-medium text-nim">
                  <span className="unified-diff-header-session-name overflow-hidden text-ellipsis whitespace-nowrap font-semibold text-[var(--nim-primary)]">{sessionInfo.sessionTitle}</span>
