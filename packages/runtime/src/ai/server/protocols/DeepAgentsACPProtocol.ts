/**
 * DeepAgentsACPProtocol -- AgentProtocol adapter for a DeepAgents ACP server.
 *
 * Spawns a subprocess running an ACP-compatible Python DeepAgents launcher and
 * speaks the ACP protocol over stdio via JSON-RPC framing.
 *
...
export interface ACPToolPermissionRequest {
  requestId: string;
  sessionId: string;
  toolName: string;
  toolTitle: string;
...
interface DeepAgentsACPProtocolDeps {
...
/**
 * Cap on retained stderr bytes. DeepAgents CLI streams `tracing-subscriber` output
...
export class DeepAgentsACPProtocol implements AgentProtocol {
  readonly platform = 'deepagents-acp';
  ...
  private readonly onPermissionRequest?: DeepAgentsACPProtocolDeps['onPermissionRequest'];
  ...
  constructor(apiKey: string, deps?: DeepAgentsACPProtocolDeps) {
    this.apiKey = apiKey;
    this.baseUrl = deps?.baseUrl ?? '';
    this.spawnProcess = deps?.spawnProcess ?? spawn;
    const defaultLaunch = DeepAgentsACPProtocol.resolveDefaultLaunchCommand();
    const envCommand = process.env.NIMBALYST_DEEPAGENTS_ACP_COMMAND;
    const envArgs = DeepAgentsACPProtocol.parseArgs(process.env.NIMBALYST_DEEPAGENTS_ACP_ARGS);
    this.command = deps?.command || envCommand || defaultLaunch.command;
    this.args = deps?.args ?? (envArgs.length > 0 ? envArgs : defaultLaunch.args);
  ...
  async createSession(options: SessionOptions): Promise<ProtocolSession> {
    ...
    try {
      ...
      console.error('[DeepAgentsACPProtocol] newSession failed:', detail);
    ...
    if (options.model) {
      await this.setSessionModelIfNeeded(connection, result.sessionId, options.model);
    }
  ...
  async resumeSession(sessionId: string, options: SessionOptions): Promise<ProtocolSession> {
    ...
    if (options.model) {
      await this.setSessionModelIfNeeded(connection, sessionId, options.model);
    }
  ...
  async *sendMessage(
    ...
    const turnState: ActiveTurnState = {
      ...
      workspacePath: typeof session.raw?.workspacePath === 'string' ? session.raw.workspacePath : undefined,
      abortSignal,
      queue,
  ...
  private async getConnection(): Promise<ACPClientConnection> {
    ...
    if (!this.connection) {
      throw new Error('Failed to initialize DeepAgents ACP connection');
    }
  ...
  private async initializeConnection(): Promise<void> {
    ...
    // `Buffer[]` accumulated for the lifetime of the DeepAgents child process,
    ...
    child.stderr.on('data', (chunk: Buffer) => {
      ...
      if (text) {
        console.log(`[DeepAgentsACPProtocol:stderr] ${text}`);
      }
    });
    ...
    child.once('exit', (code, signal) => {
      ...
      const exitReason =
        code !== null
          ? `DeepAgents ACP process exited with code ${code}`
          : `DeepAgents ACP process exited with signal ${signal ?? 'unknown'}`;
    ...
    try {
      ...
      this.destroy();
      throw new Error(
        `Failed to initialize DeepAgents ACP agent "${this.command}": ${this.formatError(
          this.processExitError ?? error,
        )}`,
      );
    }
  ...
  private normalizeArguments(rawInput: unknown): Record<string, unknown> | undefined {
    ...
    if (typeof rawInput === 'object' && !Array.isArray(rawInput)) {
      return DeepAgentsACPProtocol.unwrapMcpArguments(rawInput as Record<string, unknown>);
    }
    return { value: rawInput };
  }

  /**
   * Some ACP edit tool calls don't put a file path in `rawInput` --
  ...
  private async buildPromptBlocks(message: ProtocolMessage) {
    ...
    // ACP doesn't have a separate system prompt field; the DeepAgentsACPProvider
  ...
  private async setSessionModeIfNeeded(
    ...
    // Map Nimbalyst permission modes to DeepAgents ACP approval presets.
    // DeepAgents exposes approval as a config option (id: "mode") rather than
    // a standard ACP session mode.
    //
    // DeepAgents modes: "ask_before_edits", "accept_edits", "accept_everything".
    let modeValue: string;
    if (permissionMode === 'bypass-all') {
      modeValue = 'accept_everything';
    } else if (permissionMode === 'allow-all') {
      modeValue = 'accept_edits';
    } else {
      modeValue = 'ask_before_edits';
    }
    ...
    try {
      ...
      console.warn('[DeepAgentsACPProtocol] setSessionConfigOption failed, trying setSessionMode:', configError);
      try {
        ...
        console.warn('[DeepAgentsACPProtocol] Failed to set session mode:', modeError);
      }
    }
  }

  private async setSessionModelIfNeeded(
    connection: ACPClientConnection,
    sessionId: string,
    model: string,
  ): Promise<void> {
    try {
      ...
      console.warn('[DeepAgentsACPProtocol] Failed to set session model:', error);
    }
  }
  ...
  private buildChildEnv(): Record<string, string> {
    ...
    for (const [key, value] of Object.entries(process.env)) {
      ...
      if (DeepAgentsACPProtocol.isCredentialEnvKey(key)) {
        continue;
      }
      env[key] = value;
    }
    ...
    if (this.baseUrl) {
      env.NIMBALYST_CLI_PROXY_BASE_URL = this.baseUrl;
    }
    if (this.apiKey) {
      env.NIMBALYST_CLI_PROXY_API_KEY = this.apiKey;
    }

    return env;
  }

  private static isCredentialEnvKey(key: string): boolean {
  ...
  private convertMcpServers(
    ...
    for (const [name, config] of Object.entries(mcpServers)) {
      ...
      if (cfg.url && typeof cfg.url === 'string') {
        ...
        // DeepAgents ACP only supports stdio MCP servers natively for the most
        ...
        const mcpRemotePath = DeepAgentsACPProtocol.resolveMcpRemoteProxy();
        if (mcpRemotePath) {
   ...
   * to stdio for agents that only support stdio transport (like DeepAgents ACP).
   */
  ...
  /**
   * Resolve a package-provided deepagents-acp binary if a future package ships
   * one. Development builds should set NIMBALYST_DEEPAGENTS_ACP_COMMAND and
   * NIMBALYST_DEEPAGENTS_ACP_ARGS to an explicit Python launcher.
   */
  static resolveDeepAgentsAcpBinary(): string | undefined {
    const binaryName = process.platform === 'win32' ? 'deepagents-acp.exe' : 'deepagents-acp';

    try {
      const require = createRequire(import.meta.url);
      const packageJsonPath = require.resolve('deepagents-acp/package.json');
  ...
  static resolveProductLauncherScript(): string | undefined {
    try {
      const candidate = fileURLToPath(new URL('./deepagents_acp_launcher.py', import.meta.url));
      if (existsSync(candidate)) {
        return candidate;
      }
    } catch {
      // import.meta.url may not be file-backed in unusual bundled test setups.
    }
    return undefined;
  }

  static resolveDefaultLaunchCommand(): LaunchCommand {
    const launcherScript = DeepAgentsACPProtocol.resolveProductLauncherScript();
    if (launcherScript) {
      return {
        command: 'uv',
        args: [
    ...
    const packagedBinary = DeepAgentsACPProtocol.resolveDeepAgentsAcpBinary();
    if (packagedBinary) {
      return { command: packagedBinary, args: [] };
    }

    return { command: 'deepagents-acp', args: [] };
  }
